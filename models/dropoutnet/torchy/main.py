...
def main():
    ...
    u_pref_scaled = np.float32(dat['u_pref_scaled'])
    print(u_pref_scaled.dtype)
    v_pref_scaled = np.float32(dat['v_pref_scaled'])
    ...
    for epoch in range(num_epoch):
        ...
        for b in utils.batch(row_index, user_batch_size):
            ...
            for (start, stop) in pbar:
                ...
                Ucontent = torch.tensor(Ucontent).to(d_train).to(torch.float32)
                Vcontent = torch.tensor(Vcontent).to(d_train).to(torch.float32)
            ...
            if n_step % eval_every == 0:
                ...
                _,recall_warm  = dropout_net.evaluate(recall_k=recall_at, eval_data=eval_warm,      device=d_eval)
                dropout_net.to(d_train)
                dropout_net.train()

                # checkpoint
                agg_cur = np.sum(recall_warm)
                agg_best = np.sum(best_warm)
                if agg_cur > agg_best:
                    best_model = dropout_net
                    torch.save(dropout_net, 'best-model.pt')
                ...
                print('warm start\t%s' % (' '.join(['%.4f' % i for i in recall_warm]) ))
    results,recall_warm = best_model.evaluate(recall_k=recall_at, eval_data=eval_warm,      device=d_eval)
...
def load_data(data_path):
    ...
    u_file                  = os.path.join(data_path, 'U.csv.bin')
    v_file                  = os.path.join(data_path, 'V.csv.bin')
    user_content_file       = os.path.join(data_path, 'user_content.dat')
    item_content_file       = os.path.join(data_path, 'item_content.dat')
    train_file              = os.path.join(data_path, 'train_interactions.csv')
    test_warm_file          = os.path.join(data_path, 'test_interaction.csv')
    test_warm_iid_file      = os.path.join(data_path, 'test_warm_item_ids.csv')

    dat = {}
    # load preference data
    timer.tic()
    u_pref = np.fromfile(u_file).reshape(n_users, 200)
    v_pref = np.fromfile(v_file).reshape(n_items, 200)
    dat['u_pref'] = np.float32(u_pref)
    dat['v_pref'] = np.float32(v_pref)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--rank', type=int, default=5, help='output rank of latent model')
